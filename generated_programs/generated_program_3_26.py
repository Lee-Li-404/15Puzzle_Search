...
def heuristic(s: State) -> int:
    ...
    It evolves the best-performing previous version (which achieved a cost_ratio of 1.612
    with an overall weighting factor of 3.75) by further increasing the overall greedy
    weighting factor. This modification aims to capitalize on the available headroom
    in the cost_ratio bound to aggressively reduce the number of generated nodes.

    The heuristic combines three well-known components:
    ...
    The primary modification is increasing the overall greedy weighting factor
    from 3.75 to 3.8. This aggressive inflation of the heuristic value is designed
    to prune the search tree even more effectively, leveraging the available headroom
    in the cost_ratio to further optimize for fewer generated nodes.

    The formula used is: `int((MD + LC * 6 + CC) * 3.8)`.
    """
    ...
    # Increased to 3.8 to aggressively prune search nodes while maintaining
    # the cost_ratio constraint, building on the 3.75 factor which yielded cost_ratio 1.612.
    return int(base_h * 3.8)