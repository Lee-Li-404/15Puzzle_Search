...
def heuristic(s: State) -> int:
    ...
    It evolves the best-performing previous versions by further increasing the
    overall greedy weighting factor. It capitalizes on the available headroom
    ...
    from 3.7 (in previous top-performing versions) to 3.75. This aggressive
    inflation of the heuristic value is designed to prune the search tree even
    more effectively, leveraging the available headroom in the cost_ratio to
    further optimize for fewer generated nodes.

    The formula used is: `int((MD + LC * 6 + CC) * 3.75)`.
    """
    ...
    # Increased to 3.75 to aggressively prune search nodes while maintaining
    # the cost_ratio constraint.
    return int(base_h * 3.75)
