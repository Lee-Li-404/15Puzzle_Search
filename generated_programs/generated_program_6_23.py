...
def heuristic(s: State) -> int:
    ...
    # Conflict if tile 15 is in place, but tile 11 (above) or tile 14 (left) are not.
    if tiles[15] == 15 and (tiles[11] != 11 or tiles[14] != 14):
        cc += 1

    # Weighting strategy:
    # This heuristic builds upon the best-performing predecessor (MD + 3.5 * LC + 0.5 * CC) * 3.2.
    # It strategically increases the weight of Linear Conflicts and the overall multiplier.
    # The previous cost_ratio of 1.737 allows for this slight increase in greediness
    # to further reduce the number of unique nodes generated, while aiming to stay
    # within the 1.80 cost_ratio bound.

    # Increased weight for linear conflicts, as they represent significant impediments.
    conflict_weight_LC = 3.6  
    # Small, guiding weight for corner conflicts.
    conflict_weight_CC = 0.5  
    # Slightly increased overall multiplier for more aggressive pruning.
    overall_weight = 3.25     
