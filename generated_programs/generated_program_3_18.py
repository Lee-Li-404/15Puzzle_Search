...
def heuristic(s: State) -> int:
    """
    This heuristic evolves the best-performing previous version (score=0.0006, cost_ratio=1.77) 
    by further amplifying the penalties for difficult-to-resolve patterns and increasing the
    ...
    1.  Increased Linear Conflict Weight: The penalty multiplier for linear
        conflicts is increased from 6 to 7. This places an even higher penalty
    ...
    2.  Increased Overall Weighting Factor: The final greedy weighting factor is
        incremented from 3.3 to 3.4. This small but significant increase makes
        ...
        to optimality. This is expected to bring the cost_ratio very close to 1.80.

    The heuristic is calculated as: `int((MD + LC * 7 + CC) * 3.4)`.
    """
    ...
    # Combine all components. Linear conflicts are weighted more heavily (lc * 7).
    base_h = md + lc * 7 + cc

    # Apply an overall greedy weighting factor to further inflate the heuristic.
    # Increased from 3.3 to 3.4 to maximize node reduction.
