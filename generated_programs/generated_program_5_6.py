...
def heuristic(s: State) -> int:
    ...
    and optimizing the linear conflict calculation. It maintains the successful combination of
    Manhattan distance (MD), linear conflicts (LC), and corner conflicts (CC).
    ...
    - Linear Conflicts (lc):
        - The calculation is optimized to iterate directly through tile positions using nested loops,
          avoiding intermediate list creations for better efficiency.
        ...
        - The linear conflict component (`lc`) is now tripled within the base heuristic calculation
          (`base_h = md + lc * 3 + cc`). This selectively increases the heuristic for states
          with these difficult-to-resolve patterns, guiding A* more effectively away from them.

    - Corner Conflicts (cc): Specific conflicts for tiles 3, 12, 15 if they are in their
      goal position but block an adjacent tile from reaching its goal position. Each adds 2 to `cc`.

    - Final Weighting: The overall weighting of 2.7 is maintained. The targeted increase in the
      base heuristic for complex states should further reduce generated nodes without drastically
      increasing the solution cost ratio beyond the 1.80 bound, given the previous comfortable margin.
    """
    MD_TABLE = tuple(
        tuple(
